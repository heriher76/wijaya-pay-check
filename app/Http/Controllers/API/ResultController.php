...
class ResultController extends Controller
{
    ...
    public function getBestRoute(Request $request)
    {
        ...
        foreach ($openList as $hintCost) {
            ...
            $fn = $gn + $hn;
            array_push($initialActiveNode, (object) array('id' => $hintCost->id, $startLat, $startLong, 'latitude' => $hintCost->latitude, 'longidute' => $hintCost->longitude, 'Gn' => $gn, 'Hn' => $hn, 'Fn' => $fn));
        }
        $activeNode = [];
        foreach ($initialActiveNode as $activatedNode) {
            if ($activatedNode->Gn < 0.06) {
                array_push($activeNode, (object) array('id' => $activatedNode->id, $startLat, $startLong, 'latitude' => $activatedNode->latitude, 'longidute' => $activatedNode->longidute, 'Gn' => $gn, 'Hn' => $hn, 'Fn' => $fn));
            } else (
                foreach($activatedNode as $actNode){

                }
            )
        }
        dd($initialActiveNode);
